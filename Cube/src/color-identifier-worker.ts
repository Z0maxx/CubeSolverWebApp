...
function translateResPixelColor(color: number): [number, number, number] {
    switch (color) {
        case 72:
            return [255, 0, 0]
        case 108:
            ...
            res.json().then(async (result: CubeResultImage) => {
                ...
                if (resSize > 0) {
                    ...
                    for (let i = 0; i < resSize; i++) {
                        const translatedColor = translateResPixelColor(resPixels[i])
                        resImage[i * 4] = translatedColor[0]
                        resImage[i * 4 + 1] = translatedColor[1]
                        resImage[i * 4 + 2] = translatedColor[2]
                        resImage[i * 4 + 3] = 255
                    }
