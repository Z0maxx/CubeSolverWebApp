...
const errorsPopup = assertHTMLDialogElement(document.getElementById('identified-colors-errors-dialog'))
...
assertExists(document.getElementById('close-identified-colors-errors')).addEventListener('click', _ => {
    errorsPopup.close()
})
...
function showIdentifiedColors() {
    for (let i = 0; i < 9; i++) {
        (squares[i] as HTMLElement).style.backgroundColor = identifiedColorStrings[i]
    }
...
async function setFrame(bitmap: ImageBitmap) {
    const promise = new Promise(resolve => {
        correctionWorker.addEventListener('message', (message: MessageEvent<CorrectionWorkerResponse>) => {
            resolve(1)
        })
